	...
	void Start () {
		...
		for (int i = 0; i < Rows + Columns; i++)
		{
			...
			if (mazeCell.HasSpike == false && mazeCell.IsGoal == false)
			{
				...
				{
					GameObject tmp = Instantiate(SpikePrefabs[Random.Range(0, SpikePrefabs.Length)], new Vector3(x, 1, z), Quaternion.Euler(0, 0, 0)) as GameObject;
					tmp.transform.parent = transform;
					mazeCell.HasSpike = true;
				} else if (mazeCell.WallLeft && mazeCell.WallRight)
				{
					GameObject tmp = Instantiate(SpikePrefabs[Random.Range(0, SpikePrefabs.Length)], new Vector3(x, 1, z), Quaternion.Euler(0, 90, 0)) as GameObject;
					tmp.transform.parent = transform;
					mazeCell.HasSpike = true;
				}
			}
